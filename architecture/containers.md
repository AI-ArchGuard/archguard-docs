...
- 状态：Accepted
- 生效日期：2026-09-09
- 决策依据：[ADR-0004](../adr/0004-platform-modular-monolith.md)、[ADR-0005](../adr/0005-postgresql-business-source-of-truth.md)、[ADR-0006](../adr/0006-java-first-phased-delivery.md)、[ADR-0007](../adr/0007-dedicated-web-client.md)

| 平面/容器 | 部署边界 | 数据所有权 | 启用阶段 |
|---|---|---|---|
| Platform | Java/Spring Boot 模块化单体 | PostgreSQL 中的业务事实 | 2 |
| Web | 独立 React 静态客户端与反向代理 | 无业务持久状态；只持有浏览器会话令牌 | 2 |
...
- 用户 → Web → Platform：OIDC + 版本化 REST；CI 可直接调用 Platform，阶段 3 增加 Git/PR/Webhook。
