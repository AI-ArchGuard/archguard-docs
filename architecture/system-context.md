...
D13 已冻结 Repository 工作区的正常立即清理和失败后 24 小时补偿上限。Platform 结果、安全审计、模型数据和备份的具体产品保留时长仍须由 D14 与运行规范基于合规和产品证据决定。
...
- D14 需要哪些额外导出 Fact，以及 Platform 结果的产品保留时长是多少？
- 安全审计、模型数据和备份的具体保留与删除时限是什么？由 D14 与运行规范决定。
