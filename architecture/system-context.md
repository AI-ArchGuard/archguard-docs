...
本文只回答谁使用 ArchGuard、哪些外部系统与其交换什么信息，以及数据跨越哪些一级信任边界。容器和进程分工由 D08 定义，七仓库依赖由 D09 定义，Scanner 内部结构由 D10 定义；字段级 Schema、Rule 所有权和安全实现由 G3 决定。
...
| 源码与工作区 | Java 源码、POM、配置、临时检出 | Scanner 仅按最小需要临时处理 | 默认任务结束后清理；失败清理与最长保留期由 G3 冻结 |
...
具体时长尚无合规或产品证据，不在 G2 虚构数值；D13 必须为源码临时副本、结果、审计、模型数据和备份分别给出清理责任与可验证上限。
...
- V1 首个 Repository 输入方式是本地受控路径、归档上传还是托管服务不可变引用？由 D14 Feature Spec 决定。
- 哪些分析结果必须持久化，哪些 Fact 只在 Scanner 内部存在？由 D11–D12 决定。
- 源码临时副本、审计和备份的具体保留与删除时限是什么？由 D13 决定。
